...
- [ ] `node: clean`：编码探测（UTF-8/GB18030）、水印/广告行清除、段落规范化、章节正则切分
- [ ] 上传页：拖入 txt → 存 R2 → 跑 clean → 章节列表（标题/字数/状态）
- [ ] 章节详情页：`cleaned_text` 展示 + 人工合并/拆分章节的编辑能力（M0 只要显示）
- [ ] 单元测试：无章节标题样本、繁体标题、GBK 文件

**DoD**：上传一个脏 txt，章节列表与原文核对无误；乱码/水印样本通过测试。
